...
sub setup_common() {
  ...
  system('sudo cp /usr/share/pyshared/speechd_config/config.py /usr/share/pyshared/speechd_config/config.py.' . $t);
  `cat /usr/share/pyshared/speechd_config/config.py.$t | sed -e 's/"espeak", /"espeak", "ekho", /' | sed -e 's/"ekho", "ekho", /"ekho", /' > /tmp/config.py.ekho`;
  `sudo mv /tmp/config.py.ekho /usr/share/pyshared/speechd_config/config.py`;
}
...
if (`grep precise /etc/lsb-release`) {
  ...
  if (! -e '/usr/lib/i386-linux-gnu/speech-dispatcher/libsdaudio.so.2') {
    `sudo cp speechd-api/src/audio/.libs/libsdaudio.so* /usr/lib/i386-linux-gnu/speech-dispatcher/`;
  }
  `sudo ln -sf /usr/lib/i386-linux-gnu/speech-dispatcher/libsdaudio.so.2 /usr/lib/libsdaudio.so.2`; # not for 14.04
  `sudo ln -s /usr/lib/python3/dist-packages/speechd_config /usr/share/pyshared/speechd_config`;
  setup_common();
  kill_speechd();
  system('sudo rm -rf /usr/local/share/ekho-data');
  system('sudo make install');
